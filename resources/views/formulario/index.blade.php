...
<div class="container border p-4 mt-4">    
    <table class="table">
        <tr>
            ...
            @if ( auth()->user()->hasRole('admin') || auth()->user()->hasRole('tutor') || auth()->user()->hasRole('comision'))
                <th>Acciones</th>
            @endif
        </tr>
    @foreach ($formularios as $formulario)    
        <tr>
            ...
            <td>
            {!! date('d/m/Y', strtotime($formulario->created_at)) !!}
            </td>
            @if ( auth()->user()->hasRole('admin') || auth()->user()->hasRole('tutor') || auth()->user()->hasRole('comision'))
            <td style="display: flex; gap: 5px">
                ...
                @if ( auth()->user()->hasRole('tutor'))
                    <form action="{{route('formulario.aceptar.tutor', $formulario->id)}}" method="POST">
                        @method('PUT')
                        @csrf
                        <button class="btn btn-primary">
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="bi bi-journal-check" viewBox="0 0 16 16">
                ...
                @if ( auth()->user()->hasRole('comision'))
                    <form action="{{route('formulario.aceptar.comision', $formulario->id)}}" method="POST">
                        @method('PUT')
                        @csrf
                        <button class="btn btn-primary">
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="bi bi-journal-check" viewBox="0 0 16 16">
                ...
                @endif
                @if ( $formulario->estado != 'rechazado' && auth()->user()->hasRole('admin'))
